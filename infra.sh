...
CONTAINER_APP_ENV_IP=`az containerapp env show -g $RESOURCE_GROUP -n $CONTAINERAPPS_ENVIRONMENT --query staticIp -o tsv`
az mysql flexible-server create --location $LOCATION --resource-group  $RESOURCE_GROUP \
  ...
  --sku-name Standard_B1s  --public-access $CONTAINER_APP_ENV_IP  --storage-auto-grow Enabled 
...
az containerapp create \
  ...
  --environment-variables "database__client=mysql,database__connection__host=$MYSQL_SERVER_NAME.mysql.database.azure.com \
